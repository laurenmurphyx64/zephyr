	       ...
	       const struct llext_load_param *ldr_parm)
{
	...
	*ext = llext_heap_alloc_metadata(llext_heap, sizeof(struct llext));
	if (*ext == NULL) {
	...
	if (ret < 0) {
		llext_heap_free_metadata(llext_heap, *ext);
		*ext = NULL;
		goto out;
	}
...
int llext_unload(struct llext **ext)
{
	...
	*ext = NULL;
	...
	if (tmp->sect_hdrs_on_heap) {
		llext_heap_free_metadata(llext_heap, tmp->sect_hdrs);
	}

	llext_free_regions(tmp);
	llext_heap_free_metadata(llext_heap, tmp->sym_tab.syms);
	llext_heap_free_metadata(llext_heap, tmp->exp_tab.syms);
	llext_heap_free_metadata(llext_heap, tmp);
