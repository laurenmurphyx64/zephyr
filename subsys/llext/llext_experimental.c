...
{
	struct llext_elf_sect_map **map = llext_heap_alloc_metadata(heap, sizeof(*map) * n_ext);
	...
	for (n_map = 0, n_exp_tab = 0; n_map < n_ext; n_map++) {
		...
		map[n_map] =
			llext_heap_alloc_metadata(llext_heap, sizeof(**map) * ext[n_map]->sect_cnt);
		if (!map[n_map]) {
	...
	struct llext_symbol **exp_tab =
		llext_heap_alloc_metadata(llext_heap, sizeof(*exp_tab) * n_exp_tab);
	...
	if (n_exp_tab) {
		...
		for (i = 0, j = 0; i < n_ext; i++) {
			if (ext[i]->exp_tab.sym_cnt) {
				...
				exp_tab[j] = llext_heap_alloc_metadata(llext_heap, size);
				if (!exp_tab[j]) {
	...
	for (i = 0, j = 0; i < n_ext; i++) {
		next = llext_heap_alloc_metadata(llext_heap, sizeof(*next));
		if (!next) {
	...
	llext_heap_free_metadata(llext_heap, map);
	llext_heap_free_metadata(llext_heap, exp_tab);
	...
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&llext_list, next, tmp, llext_list) {
		for (i = 0; i < n_ext; i++) {
			if (ext[i] == next) {
				...
				llext_heap_free_metadata(llext_heap, next);
				ext[i] = NULL;
				break;
			}
		}
...
free_sym:
	for (i = 0; i < n_exp_tab && exp_tab[i]; i++) {
		llext_heap_free_metadata(llext_heap, exp_tab[i]);
	}
...
free_map:
	for (i = 0; i < n_map; i++) {
		llext_heap_free_metadata(llext_heap, map[i]);
	}
