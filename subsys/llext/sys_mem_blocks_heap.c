...
uint8_t llext_data_heap_buf[CONFIG_LLEXT_DATA_HEAP_SIZE * KB(1)] __used
	__aligned(CONFIG_LLEXT_HEAP_SYS_MEM_BLOCK_BLOCK_SIZE)
	__attribute__((section(".rodata.llext_data_heap")));
...
struct llext_sys_mem_blocks_heap llext_sys_mem_blocks_heap = Z_LLEXT_SYS_MEM_BLOCKS_HEAP();
struct llext_heap *llext_heap = &llext_sys_mem_blocks_heap.heap;
