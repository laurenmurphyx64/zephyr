...
BUILD_ASSERT(CONFIG_LLEXT_HEAP_SYS_MEM_BLOCKS_BLOCK_SIZE % LLEXT_PAGE_SIZE == 0,
	     "CONFIG_LLEXT_HEAP_SYS_MEM_BLOCKS_BLOCK_SIZE must be multiple of LLEXT_PAGE_SIZE");
SYS_MEM_BLOCKS_DEFINE(llext_ext_heap, CONFIG_LLEXT_HEAP_SYS_MEM_BLOCKS_BLOCK_SIZE,
...
void *llext_sys_mem_blocks_heap_aligned_alloc_data_instr(struct llext_heap *h, struct llext *ext,
...
void *llext_sys_mem_blocks_heap_aligned_alloc_data(struct llext_heap *h, struct llext *ext,
						   size_t align, size_t bytes)
{
	return llext_sys_mem_blocks_heap_aligned_alloc_data_instr(h, ext, &llext_data_heap, align,
								  bytes);
}
...
void *llext_sys_mem_blocks_heap_aligned_alloc_instr(struct llext_heap *h, struct llext *ext,
						    size_t align, size_t bytes)
{
	return llext_sys_mem_blocks_heap_aligned_alloc_data_instr(h, ext, &llext_instr_heap, align,
								  bytes);
}
...
void llext_sys_mem_blocks_heap_free_data_instr(struct llext_heap *h, struct llext *ext,
...
{
	llext_sys_mem_blocks_heap_free_data_instr(h, ext, &llext_data_heap, ptr);
}

void llext_sys_mem_blocks_heap_free_instr(struct llext_heap *h, struct llext *ext, void *ptr)
{
	llext_sys_mem_blocks_heap_free_data_instr(h, ext, &llext_instr_heap, ptr);
}

struct llext_sys_mem_blocks_heap llext_sys_mem_blocks_heap_inst = Z_LLEXT_SYS_MEM_BLOCKS_HEAP();
struct llext_heap *llext_heap_inst = &llext_sys_mem_blocks_heap_inst.heap;
