			      ...
			      enum llext_mem mem_idx, const struct llext_load_param *ldr_parm)
{
	...
	region_alloc = ROUND_UP(region_alloc, CONFIG_LLEXT_HEAP_SYS_MEM_BLOCKS_BLOCK_SIZE);
#endif
...
err:
	if (region->sh_flags & SHF_EXECINSTR) {
		llext_heap_free_instr(ext, ext->mem[mem_idx]);
	} else {
		llext_heap_free_data(ext, ext->mem[mem_idx]);
	}
	ext->mem[mem_idx] = NULL;
	return ret;
}
