...
static int llext_load_elf_data(struct llext_loader *ldr, struct llext *ext)
{
	...
	ldr->sect_map = llext_heap_alloc_metadata(llext_heap, sect_map_sz);
	if (!ldr->sect_map) {
	...
	if (ext->sect_hdrs) {
		...
		ext->sect_hdrs = llext_heap_alloc_metadata(llext_heap, sect_hdrs_sz);
		if (!ext->sect_hdrs) {
...
static int llext_allocate_symtab(struct llext_loader *ldr, struct llext *ext)
{
	...
	sym_tab->syms = llext_heap_alloc_metadata(llext_heap, syms_size);
	if (!sym_tab->syms) {
		return -ENOMEM;
	}
				...
				const struct llext_load_param *ldr_parm)
{
	...
	exp_tab->syms = llext_heap_alloc_metadata(llext_heap,
...
out:
	...
	if (!(IS_ENABLED(CONFIG_LLEXT_LOG_LEVEL_DBG) && ret == 0)) {
		llext_heap_free_metadata(llext_heap, ext->sym_tab.syms);
		ext->sym_tab.sym_cnt = 0;
	...
	if (ret != 0) {
		...
		llext_heap_free_metadata(llext_heap, ext->exp_tab.syms);
...
int llext_free_inspection_data(struct llext_loader *ldr, struct llext *ext)
{
	if (ldr->sect_map) {
		...
		llext_heap_free_metadata(llext_heap, ldr->sect_map);
		ldr->sect_map = NULL;
	}
