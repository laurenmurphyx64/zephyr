...
int applet_init(struct applet *applet_inst, const char *name,
		   const struct applet_opts *opts)
{
			       ...
			       const char *thread_name)
{
	...
	LOG_DBG("applet '%s': thread %p pinned to CPU %d",
		applet_inst->name, slot->thread, applet_inst->opts.cpu);
#endif
			     ...
			     const char *thread_name)
{
	...
	llext_entry_fn_t fn = (llext_entry_fn_t)llext_find_sym(
		&applet_inst->ext->exp_tab, entry_sym);

	if (fn == NULL) {
		LOG_ERR("applet '%s': entry symbol '%s' not found",
			applet_inst->name, entry_sym);
		return -ENOENT;
	}
...
int applet_start(struct applet *applet_inst)
{
	...
	LOG_DBG("applet '%s': started (%u thread%s)",
		applet_inst->name, applet_inst->thread_count,
