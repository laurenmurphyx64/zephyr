...
#if defined(CONFIG_APPLET_LLEXT) && defined(CONFIG_LLEXT_STORAGE_WRITABLE)
/*
...
APPLET_THREAD_STACK_ARRAY_DEFINE(stacks_b, POOL_B_THREADS,
				 CONFIG_APPLET_THREAD_STACK_SIZE_DEFAULT);
...
#define APPLET_TEST_CASE(_name, _img)                                                              \
	{                                                                                          \
		.name = (_name), .image = &(_img), .opts = APPLET_OPTS_DEFAULT, .pool = POOL_A,    \
		.num_threads = 1,                                                                  \
	}
...
#else
	ret = applet_add_thread(&applet_1, pool_stack(POOL_A, 1), POOL_STACK_SIZE,
				img_worker.entry, NULL, NULL);
#endif
...
ZTEST(applet, test_invalid_args)
{
	...
	zassert_equal(applet_add_thread(NULL, pool_stack(POOL_A, 0), POOL_STACK_SIZE,
		      ...
		      -EINVAL);
	zassert_equal(applet_add_thread(&applet_1, NULL, POOL_STACK_SIZE,
					(k_thread_entry_t)k_yield, NULL, NULL),
		      -EINVAL);
...
ZTEST(applet, test_thread_slot_exhaustion)
{
	...
	for (unsigned int round = 0; round < ARRAY_SIZE(round_count); round++) {
		...
		while (added < APPLET_TEST_SLOT_LIMIT) {
			int ret = applet_add_thread(&applet_1, pool_stack(POOL_A, 0),
						    POOL_STACK_SIZE, (k_thread_entry_t)k_yield,
						    NULL, NULL);
...
ZTEST(applet, test_llext_spawn)
{
	...
	int ret = applet_spawn(&applet_1, "spawn", elf_storage(&applet_1, worker_elf, sizeof(worker_elf)),
...
ZTEST(applet, test_llext_load_then_start)
{
	...
	int ret = applet_load(&applet_1, "load", elf_storage(&applet_1, worker_elf, sizeof(worker_elf)),
			      sizeof(worker_elf), pool_stack(POOL_A, 0), POOL_STACK_SIZE, &opts);
...
ZTEST(applet, test_llext_two_entry_points)
{
	...
	zassert_ok(applet_add_thread_sym(&applet_1, pool_stack(POOL_A, 0), POOL_STACK_SIZE,
					 img_worker.sym,
					 (void *)(uintptr_t)APPLET_TEST_CMD_SLEEP, "worker"));
...
ZTEST(applet, test_halt_thread_on_fault)
{
	...
	zassert_ok(fault_applet_setup(&applet_1, "halt-thread", APPLET_HALT_ON_FAULT_THREAD,
				      POOL_A));
	zassert_ok(applet_start(&applet_1));
...
ZTEST(applet, test_halt_applet_on_fault)
{
	...
	zassert_ok(fault_applet_setup(&applet_1, "halt-applet", APPLET_HALT_ON_FAULT_APPLET,
				      POOL_A));
	zassert_ok(applet_start(&applet_1));
