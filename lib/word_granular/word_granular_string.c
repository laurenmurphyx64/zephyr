...
static inline void copy_byte_to_word_granular(unsigned char *dst, const unsigned char *src)
{
...
static inline void copy_byte_from_word_granular(unsigned char *dst, const unsigned char *src)
{
	/* Mask off the lower bits to get an aligned pointer */
	const mem_word_t mask = sizeof(mem_word_t) - 1;
...
static inline void set_byte_word_granular(unsigned char *dst, unsigned char value)
{
	const mem_word_t mask = sizeof(mem_word_t) - 1;
	mem_word_t d_aligned = (mem_word_t)dst & ~mask;
	unsigned int d_offset = (mem_word_t)dst & mask;
...
void *memset_word_granular(void *buf, int c, size_t n)
{
	...
	while (((mem_word_t)d_byte) & (sizeof(mem_word_t) - 1)) {
		...
		set_byte_word_granular(d_byte++, c_byte);
		n--;
	}
...
void *memcpy_to_word_granular(void *d, const void *s, size_t n)
{
	...
	if ((((mem_word_t)d ^ (mem_word_t)s_byte) & mask) == 0) {
		...
		while (((mem_word_t)d_byte) & mask) {
			...
			copy_byte_to_word_granular(d_byte++, s_byte++);
			n--;
		}
...
void *memcpy_from_word_granular(void *d, const void *s, size_t n)
{
	...
	if ((((mem_word_t)d ^ (mem_word_t)s_byte) & mask) == 0) {
		...
		while (((mem_word_t)d_byte) & mask) {
			...
			copy_byte_from_word_granular(d_byte++, s_byte++);
			n--;
		}
