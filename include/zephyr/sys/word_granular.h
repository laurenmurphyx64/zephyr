 ...
 * @file
 *
 * @brief Public word granular memory APIs.
 * @defgroup word_granular Word Granular Memory APIs
 ...
 * @typedef mem_word_t
 *
 * @brief Access type for word-granular memory.
 *
...
/**
 * @brief Memset buffer in word-granular memory
 *
 * Memset buffer in word-granular memory.
 *
 ...
 */
void *memset_word_granular(void *buf, int c, size_t n);

/**
 * @brief Memcpy buffer into word-granular memory
 *
 * Memcpy used to copy buffer into word-granular memory.
 *
 ...
 */
void *memcpy_to_word_granular(void *d, const void *s, size_t n);

/**
 * @brief Memcpy buffer out of word-granular memory
 *
 * Memcpy used to copy buffer out of word-granular memory.
 *
 * Note that if the start of the buffer being written to or read from is not
 * word-aligned, or the length is not a multiple of the word size, bytes outside
 * of the requested range may need to be read from or written to. The data in these
 ...
 * @returns the provided destination buffer pointer, or `NULL` on error
 */
void *memcpy_from_word_granular(void *d, const void *s, size_t n);
