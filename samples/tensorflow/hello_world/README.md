...
x values to print alongside the y values predicted by the model. The x values iterate from 0 to a approximation of 2 *  Pi.
...
(Output continues indefnitely.)

Hit `Ctrl+A X` to exit QEMU.

## Deploy to Litex VexRiscv on Renode

Renode is an emulator that can be used to run the sample application on an emulated Litex VexRiscv board.
...
Download the Renode package for your OS from their 
[GitHub releases](https://github.com/renode/renode/releases/tag/v1.11.0)
and install it following the instructions in their 
[GitHub README](https://github.com/renode/renode/blob/master/README.rst).
